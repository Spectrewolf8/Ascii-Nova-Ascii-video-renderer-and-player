...
class AsciiVideoPlayer:
    ...
    def playAsciiVideo(self, gzippedJsonfile_path):
        asciiVideoDict = compress_json.load(
            gzippedJsonfile_path
    ...
    def render_overlay(self, show_fps_switch=True):
        ...
        overlay_surface.set_colorkey(
            (0, 0, 0)
        )  # Set the background color of the overlay as transparent
        pygame.draw.rect(
            overlay_surface, (30, 30, 20), overlay_surface.get_rect(), border_radius=8
        )  # Set the color and border radius
        ...
        )  # Customize the position of the media controls text
        ...
        if show_fps_switch:
            ...
            overlay_surface.blit(
                fps_surface, (5, 20)
            )  # Customize the position of the FPS text

        self.screen.blit(
            overlay_surface, (self.screen.get_rect().left + 10, 10)
        )  # Customize the position of the overlay surface on the screen
        ...
        try:
            ...
            while running:
                if playback_paused is True:
                    ...
                    )  # filling background on resize/refresh
                ...
                self.clock.tick(
                    FPS_LOCK_VALUE
                )  # making fps constant(synced to original video's fps)
